...
def before_cursor_execute(conn,cursor,statement,parameters,context,
                          executemany):
    # logging.info("Received statement: %s", statement)
    pass
...
class SQLAlchemy(BaseSQLAlchemy):
    ...
    def init_app(self,app):
        ...
        self.event.listens_for(engine.Engine,"before_cursor_execute")(before_cursor_execute)
        if database_uri.startswith('sqlite:'):
            self.event.listens_for(engine.Engine,"connect")(set_sqlite_pragma)
            self.Model.metadata = sqllite_metadata
